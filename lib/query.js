...
module.exports = {
  ...
  addRangeFilters: addRangeFilters,
...
function QueryPromise(query, func) {
  ...
  self.range = function (value) {
    addRangeFilters(self.q, value);
    return self;
  };

  self.notInRange = function (value) {
    addRangeFilters(self.q, null, value);
    return self;
  };
...
function parseRequest(indices, types, matches, options) {
  ...
  if (options.range || options.notInRange) {
    addRangeFilters(req, options.range, options.notInRange);
  }
...
function addRangeFilters(req, mustFilters, notFilters) {
  var filter = {
    bool: {
      must: [],
      must_not: [],
  ...
  var makeRangeFilter = function (v, k) {
    var f = {};
    f.range = {};
    f.range[k] = v;
    return f;
  ...
  filter.bool.must = _.map(mustFilters, makeRangeFilter);
  filter.bool.must_not = _.map(notFilters, makeRangeFilter);
